...
class MakeVideo(object):
    ...
    def make_video(self):
        # cmd = '{bin} -i "{rtmp_url}" -r 25 -s 528x312 -filter:v "crop=1440:900:240:90" -f mp4 -t {duration} -y {outfile}'
        # cmd = '{bin} -i "{rtmp_url}" -filter:v "crop=1200:500:450:350" -r 15 -t {time} -y {outfile}'
        command = [
            '{rtmp_bin} -i "{rtmp_url}" -r 25 -s 528x312 -filter:v "crop=1440:900:240:90" -f mp4 -t {duration} -y {outfile}'.format(**self.config)]
